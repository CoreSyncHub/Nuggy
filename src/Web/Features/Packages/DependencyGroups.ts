...
export class DependencyGroups extends LitElement {
  ...
  static styles = css`
    ...
    summary { cursor: pointer; text-transform: uppercase; font-size: 11px; color: var(--vscode-descriptionForeground); }
    .group { margin: 4px 0 4px 8px; }
    .tfm { font-weight: 600; }
    .dep { margin-left: 18px; color: var(--vscode-descriptionForeground); }
  `;
        ...
        ${this.groups.length === 0
          ...
          : this.groups.map(
              (g) => html`<div class="group">
                <div class="tfm">${g.targetFramework || this.i18n.t('packages.dependencies.allFrameworks')}</div>
...
declare global { interface HTMLElementTagNameMap { 'dependency-groups': DependencyGroups; } }
