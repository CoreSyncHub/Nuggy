...
import { GetPackageUpdateInfoQueryHandler, compareVersionsDesc } from '../GetPackageUpdateInfoQueryHandler';
import { PackageMetadataCache } from '@Infrastructure/NuGet/PackageMetadataCache';
import { NuGetApiError, type NuGetV3ApiClient, type RegistrationLeaf, type SearchResult } from '@Infrastructure/NuGet/NuGetV3ApiClient';
...
const LEAVES: RegistrationLeaf[] = [
  {
    version: '1.0.2', publishedUtc: '2019-04-21T14:44:00Z', listed: true,
    dependencyGroups: [{ targetFramework: '.NETStandard2.0', dependencies: [{ id: 'Newtonsoft.Json', versionRange: '[12.0.1, )' }] }],
  },
  {
    version: '2.0.0', publishedUtc: '2024-06-08T10:00:00Z', listed: true,
    dependencyGroups: [{ targetFramework: 'net6.0', dependencies: [] }],
  },
  {
    version: '2.1.0-beta.1', publishedUtc: '2024-09-01T10:00:00Z', listed: true,
    dependencyGroups: [],
  },
...
const SEARCH: SearchResult = {
  verified: true, authors: 'James Newton-King', owners: ['jamesnk'],
  totalDownloads: 47312456, iconUrl: 'https://example.test/icon',
  projectUrl: 'https://example.test/project', licenseUrl: 'https://licenses.nuget.org/MIT',
  tags: ['bson'],
};
...
describe('GetPackageUpdateInfoQueryHandler', () => {
  ...
  it('détecte les packages Microsoft via owners', async () => {
    const handler = createHandler({
      getRegistrationLeaves: jest.fn().mockResolvedValue(LEAVES),
      searchPackage: jest.fn().mockResolvedValue({ ...SEARCH, owners: ['Microsoft', 'dotnetframework'] }),
    });
    const dto = await handler.Handle(new GetPackageUpdateInfoQuery('X', '/Solution/MySolution.sln'));
  ...
  it("ne détecte PAS Microsoft sur simple correspondance de sous-chaîne dans authors ('NotMicrosoft Ltd')", async () => {
    const handler = createHandler({
      getRegistrationLeaves: jest.fn().mockResolvedValue(LEAVES),
      searchPackage: jest.fn().mockResolvedValue({ ...SEARCH, owners: [], authors: 'NotMicrosoft Ltd' }),
    });
    const dto = await handler.Handle(new GetPackageUpdateInfoQuery('X', '/Solution/MySolution.sln'));
    expect(dto.isMicrosoft).toBe(false);
  });
  ...
  it("détecte Microsoft via authors quand c'est un jeton exact parmi plusieurs ('Microsoft, aspnet')", async () => {
    const handler = createHandler({
      getRegistrationLeaves: jest.fn().mockResolvedValue(LEAVES),
      searchPackage: jest.fn().mockResolvedValue({ ...SEARCH, owners: [], authors: 'Microsoft, aspnet' }),
    });
    const dto = await handler.Handle(new GetPackageUpdateInfoQuery('X', '/Solution/MySolution.sln'));
    expect(dto.isMicrosoft).toBe(true);
  });
