...
describe('NuGetV3ApiClient', () => {
  ...
  it('le service index est mis en cache (un seul fetch pour deux appels)', async () => {
    ...
    const indexCalls = fetchMock.mock.calls.filter((c) => c[0] === 'https://api.nuget.org/v3/index.json');
  ...
  it('404 sur la registration → NuGetApiError NotFound', async () => {
    ...
    await expect(client.getRegistrationLeaves('Paquet.Prive')).rejects.toMatchObject({ kind: 'NotFound' });
  });
  ...
  it('pagination : une page sans items déclenche un fetch de la page dédiée', async () => {
    const pageUrl = 'https://api.nuget.org/v3/registration5-gz-semver2/big.package/page/1.0.0/2.0.0.json';
  ...
  it('le service index est retenté après un échec réseau', async () => {
    ...
    await expect(client.getRegistrationLeaves('Newtonsoft.Json.Bson')).rejects.toMatchObject({ kind: 'Offline' });
