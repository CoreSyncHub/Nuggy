...
export class TfmCompatibilityService {
  ...
  private accepts(project: NuGetFramework, pkg: NuGetFramework): boolean {
    ...
    if (project.family === pkg.family) {
      if (!project.versionAtLeast(pkg)) return false;
      if (pkg.platform === undefined) return true;
      return project.platform === pkg.platform;
    }

    // Le package cible une plateforme spécifique d'une autre famille → jamais compatible
    if (pkg.platform !== undefined) return false;

    switch (project.family) {
      case 'net5plus':
        if (pkg.family === 'netcoreapp') return true; // netcoreapp3.1- consommable par net5+
        if (pkg.family === 'netstandard') return pkg.major < 2 || (pkg.major === 2 && pkg.minor <= 1);
        return false;
      case 'netcoreapp':
        if (pkg.family !== 'netstandard') return false;
        // netcoreapp3.x ⊇ netstandard2.1- ; 2.x ⊇ 2.0- ; 1.x ⊇ 1.6-
        if (project.major >= 3) return pkg.major < 2 || (pkg.major === 2 && pkg.minor <= 1);
        if (project.major === 2) return pkg.major < 2 || (pkg.major === 2 && pkg.minor === 0);
        return pkg.major === 1 && pkg.minor <= 6;
      case 'netframework': {
        if (pkg.family !== 'netstandard') return false;
        const maxNetstandard = this.maxNetstandardForNetFramework(project);
        if (maxNetstandard === null) return false;
  ...
  private maxNetstandardForNetFramework(fw: NuGetFramework): [number, number] | null {
    ...
    if (v >= 461) return [2, 0]; // net461+ → netstandard2.0
    if (v >= 460) return [1, 3];
    if (v >= 451) return [1, 2]; // net451/net452
    if (v >= 450) return [1, 1]; // net45
