export type FrameworkFamily = 'netframework' | 'netstandard' | 'netcoreapp' | 'net5plus' | 'unknown';
...
export class NuGetFramework {
  ...
  public static parse(tfm: string): NuGetFramework {
    ...
    if (legacy) {
      return new NuGetFramework('netframework', Number(legacy[1]), Number(legacy[2]), Number(legacy[3] ?? 0));
    }
