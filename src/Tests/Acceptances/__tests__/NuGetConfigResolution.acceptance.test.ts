...
<configuration>
  ...
  <packageSourceMapping>
    ...
    <packageSource key="CompanyFeed">
      ...
      const contosoSources = resolver.getAllowedSourcesForPackage(resolution, "Contoso.Core.Api");
      ...
      const newtonsoftSources = resolver.getAllowedSourcesForPackage(resolution, "Newtonsoft.Json");
      expect(newtonsoftSources).toHaveLength(0); // Not allowed (no mapping)

      // Verify canSourcePackageFrom
      expect(
        resolver.canSourcePackageFrom(resolution, "Microsoft.AspNetCore.Mvc", "nuget.org"),
      ).toBe(true);
      expect(
        resolver.canSourcePackageFrom(resolution, "Microsoft.AspNetCore.Mvc", "CompanyFeed"),
      ).toBe(false);

      expect(resolver.canSourcePackageFrom(resolution, "Contoso.Api", "CompanyFeed")).toBe(true);
      expect(resolver.canSourcePackageFrom(resolution, "Contoso.Api", "nuget.org")).toBe(false);
    });
  });
