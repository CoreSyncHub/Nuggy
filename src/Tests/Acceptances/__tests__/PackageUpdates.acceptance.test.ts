...
const REGISTRATION = {
  items: [{
    items: [
      { catalogEntry: { version: '3.1.0', listed: true, dependencyGroups: [{ targetFramework: '.NETStandard2.0', dependencies: [] }] } },
      { catalogEntry: { version: '4.3.0', listed: true, dependencyGroups: [{ targetFramework: 'net6.0', dependencies: [] }] } },
    ],
  }],
};
const SEARCH = { data: [{ verified: true, authors: 'Serilog Contributors', owners: ['serilog'], tags: [] }] };
...
describe('Acceptance: Package updates (Epic 2)', () => {
  beforeEach(() => {
    ...
    mockFs.statSync.mockImplementation((p) => ({ isFile: () => (p as string) in files }) as fs.Stats);

    jest.spyOn(globalThis, 'fetch').mockImplementation(async (url) => {
      const u = String(url);
      const body = u.includes('index.json') && u.includes('api.nuget.org') ? SERVICE_INDEX
        : u.startsWith('https://registration.test/') ? REGISTRATION
        : SEARCH;
